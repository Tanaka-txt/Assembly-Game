...
/* INSTRUÇÃO NOVA (AVG): Calcula a Média Aritmética Inteira entre dois registradores */
// Operação: Rx <- (Ry + Rz) / 2
// Nota: O processador trabalha apenas com inteiros. Se a média der decimal (ex: 3.5), o valor é truncado para baixo (vira 3)
#define AVG 38      // "100110"; -- AVG Rx Ry Rz   			-- Rx <- (Ry + Rz) / 2 	Format: < inst(6) | Rx(3) | Ry(3) | Rz(3)| x >
	...
	switch(state)
	{
		...
		case STATE_DECODE:
			...
			switch(opcode){
				...
				/* INSTRUÇÃO NOVA: AVG (38) */
				case AVG:
					// Adicionamos a AVG neste bloco porque ela usa o mesmo datapath das instruções aritméticas padrão:
					// 1. Pega os operandos Ry e Rz;
					// 2. Passa pela ULA;
					// 3. Escreve o resultado em Rx.
					
					selM3 = ry; // Seleciona o valor do registrador Ry para entrar na ULA
					selM4 = rz; // Seleciona o valor do registrador Rz para entrar na ULA
					selM2 = sULA; // Configura o Mux2 para pegar a saída da ULA
					
					carry = pega_pedaco(IR,0,0);
					OP = opcode; // Envia o código da operação (38) para a ULA saber o que fazer
					
					LoadReg[rx] = 1; // Habilita a escrita do resultado no registrador de destino (Rx)
					selM6 = sULA;    // Atualiza as flags (Zero, Overflow, etc)
					LoadFR = 1;
					// -----------------------------
					state=STATE_FETCH;
					break;
	...
	switch(pega_pedaco(OP, 5, 4)) {
		case ARITH:
			switch(OP) {
				...
				/* INSTRUÇÃO NOVA: Implementação lógica da média (average) */
				case AVG:
					{
						// Soma os dois valores de entrada (Ry + Rz)
						// Usamos uma variável temporária para checar overflow antes da divisão
						unsigned int temp_sum = x + y;

						// Verificação de overflow aritmético
						// Se a soma de (x+y) for maior que 65535, o registrador estourou
						// Marcamos a flag para o programador saber que houve erro no cálculo
						if (temp_sum > MAX_VAL)
							auxFRbits[ARITHMETIC_OVERFLOW] = 1;
						else
							auxFRbits[ARITHMETIC_OVERFLOW] = 0;
						
						// Divisão por 2 usando bitwise shift
						// Dividir por potências de 2 pode ser feito deslocando bits
						// "Shift Right" (>>) desloca os bits para a direita, o que equivale a dividir por 2
						result = temp_sum >> 1; 
					}
					break;
